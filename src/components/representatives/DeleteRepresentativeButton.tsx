...
type DeleteRepresentativeButtonProps = {
...
export default function DeleteRepresentativeButton({
  ...
  representativeName,
  storagePath,
}: DeleteRepresentativeButtonProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (storagePath) {
        ...
        if (storageError) {
          console.error(
            "Representative deleted, but Storage cleanup failed:",
            storageError,
          );
        }
      }
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Unable to delete this representative.",
      );
    } finally {
      setIsDeleting(false);
    }
          ...
          <div className="w-full max-w-md rounded-2xl border border-violet-100 bg-white p-6 shadow-2xl">
            ...
            {storagePath && (
              <p className="mt-2 text-sm leading-6 text-slate-500">
                ...
                uploaded portrait will also
                be removed from Supabase
                Storage.
              </p>
            )}
